...
🚧 In active development. **Playable today:** the arcade shell (menu, calibration, countdown, HUD, game-over) with three complete modes — **Hole-in-the-Wall** and **Dodge** (standing) and **Hand Simon-Says** (seated) — plus the full juice layer (audio + effects), a downloadable PNG result card, a local daily/all-time leaderboard, a reactive mascot, an always-moving procedural attract figure on the menu, and a branded how-to-play start screen. The remaining submission to-dos are content captures: a refreshed gameplay GIF and short demo video, plus a fresh-machine smoke test of the live link. See [IMPLEMENTATION.md](IMPLEMENTATION.md) for the phase-by-phase plan and a live build-status table.
